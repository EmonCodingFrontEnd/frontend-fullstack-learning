      ...
      if (path && path.length === 3) {
        this.$http({
          url: this.$http.adornGatewayUrl(
            `/api/product/attrgroup/list/${path[path.length - 1]}`
          ),
          method: 'get',
    ...
    init(id) {
      ...
      this.$nextTick(() => {
        ...
        if (this.dataForm.attrId) {
          this.$http({
            url: this.$http.adornGatewayUrl(
              `/api/product/attr/info/${this.dataForm.attrId}`
            ),
            method: 'get',
            ...
            if (data && data.code === 0) {
              ...
              // attrGroupId
              // catelogPath
              this.categoryPath = data.attr.catelogPath
              this.$nextTick(() => {
    ...
    dataFormSubmit() {
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          this.$http({
            url: this.$http.adornGatewayUrl(
              `/api/product/attr/${!this.dataForm.attrId ? 'save' : 'update'}`
            ),
            method: 'post',
