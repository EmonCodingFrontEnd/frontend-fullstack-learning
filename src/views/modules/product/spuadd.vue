...
export default {
  ...
  methods: {
    ...
    getMemberLevels() {
      this.$http({
        url: this.$http.adornGatewayUrl('/api/member/memberlevel/list'),
        method: 'get',
    ...
    generateSkus() {
      ...
      console.log('选择的属性', selectValues)
    ...
    getShowSaleAttr() {
      ...
      if (!this.dataResp.steped[1]) {
        this.$http({
          url: this.$http.adornGatewayUrl(
            `/api/product/attr/sale/list/${this.spu.catalogId}`
          ),
          method: 'get',
          params: this.$http.adornParams({
            page: 1,
    ...
    showBaseAttrs() {
      if (!this.dataResp.steped[0]) {
        this.$http({
          url: this.$http.adornGatewayUrl(
            `/api/product/attrgroup/${this.spu.catalogId}/withattr`
          ),
          method: 'get',
          params: this.$http.adornParams({})
        }).then(({ data }) => {
    ...
    submitSkus() {
      ...
      this.$confirm('将要提交商品数据，需要一小段时间，是否继续?', '提示', {
        ...
        .then(() => {
          this.$http({
            url: this.$http.adornGatewayUrl('/api/product/spuinfo/save'),
            method: 'post',
    ...
    descartes(list) {
      ...
      return list.reduce((accumulator, currentValue) => {
        ...
        accumulator.forEach(item => {
          currentValue.forEach(secondItem => {
            if (item instanceof Array) {
              res.push([...item, secondItem])
            } else {
              res.push([item, secondItem])
            }
          })
